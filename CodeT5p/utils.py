...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        if is_sample:
            logger.info("Sample 200 data for computing bleu from %s", filename)
        else:
...
def load_and_cache_whether_to_log_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if os.path.exists(cache_fn):
        ...
        features = pool.map(convert_whether_to_log_examples_to_features, tqdm(tuple_examples, total=len(tuple_examples)))
        all_source_ids = torch.tensor([f.source_ids for f in features], dtype=torch.long)
...
def load_and_cache_multi_gen_data(args, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        task_list = ['summarize', 'translate', 'www_log', 'concode', 'defect']
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        data_dir = '{}/{}'.format(data_root, task)
        train_fn = '{}/train_remove_no_dup.code,{}/train_no_dup.code'.format(data_dir, data_dir)
        dev_fn = '{}/valid_remove_sub_block.code,{}/valid_sub_block.code'.format(data_dir, data_dir)
        test_fn = '{}/test_remove_sub_block.code,{}/test_sub_block.code'.format(data_dir, data_dir)
    elif task == 'translate':
        data_dir = '{}/{}'.format(data_root, task)
        if sub_task == 'cs-java':
...
def read_examples(filename, data_num, task):
    read_example_dict = {
        ...
        'www_log': read_www_log_examples,
        'translate': read_translate_examples,
        'concode': read_concode_examples,
        'whether_to_log': read_whether_to_log_examples,
