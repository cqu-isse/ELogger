...
class Whether_to_log_Model(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        super(Whether_to_log_Model, self).__init__()
    ...
    def forward(self, source_ids=None, labels=None):
        ...
        if (self.args.model_type == 'codet5') or (self.args.model_type == 'codet5p'):
            vec = self.get_t5_vec(source_ids)

        # logits = self.classifier(svec)
        logits1, logits2, logits3=self.classifier(vec)
