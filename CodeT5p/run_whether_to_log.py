...
from models import Whether_to_log_Model
import logging
import argparse
import math
import numpy as np
...
from utils import get_filenames, get_elapse_time, load_and_cache_whether_to_log_data
    ...
    if write_to_pred:
        with open(os.path.join(args.output_dir, "predictions.txt"), 'w') as f:
            for example, pred in zip(eval_examples, y_preds):
                if pred:
                    f.write(example.source + '\t' + '1' + '\n')
                else:
                    f.write(example.source + '\t' + '0' + '\n')
...
def main():
    ...
    model = Whether_to_log_Model(model, config, tokenizer, args)
    ...
    if args.do_train:
        ...
        train_examples, train_data = load_and_cache_whether_to_log_data(args, args.train_filename, pool, tokenizer, 'train',
                                                               is_sample=False)
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(bar):
                ...
                if (step + 1) % save_steps == 0 and args.do_eval:
                    ...
                    eval_examples, eval_data = load_and_cache_whether_to_log_data(args, args.dev_filename, pool, tokenizer,
    ...
    if args.do_test:
        ...
        for criteria in ['best-f1']:
            ...
            eval_examples, eval_data = load_and_cache_whether_to_log_data(args, args.test_filename, pool, tokenizer, 'test',
                                                                 False)
