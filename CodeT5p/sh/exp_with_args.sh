...
elif [[ $MODEL_TAG == codet5p ]]; then
  MODEL_TYPE=codet5p
  TOKENIZER=Salesforce/Codett5p-220m
  MODEL_PATH=Salesforce/Codett5p-220m
...
elif [[ ${TASK} == 'whether_to_log' ]]; then
  RUN_FN=${WORKDIR}/run_whether_to_log.py
else
  RUN_FN=${WORKDIR}/run_www_log.py
fi
