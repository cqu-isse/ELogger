...
from model_4_UniXcoder import Model

os.environ["CUDA_VISIBLE_DEVICES"]="1,3" # 使用编号为0,1号的GPU
...
def convert_examples_to_features(item):
    ...
    func,label,tokenizer, args=item
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path, pool=None):
        self.examples = []
        self.args=args
        func_filename=file_path
        
        data = []
        f = open(func_filename)
        with open(func_filename) as f:
            for line in f:
                line = line.strip()
                cons =line.split('\t')
                func = cons[0]
                ...
                if label == '0':
                    ...
                    label = 1
                data.append((func,label,tokenizer, args))
    ...
    def __getitem__(self, i):
        input_ids_tensor = torch.tensor(self.examples[i].input_ids)
        label_tensor = torch.tensor(self.examples[i].label)
...
def evaluate(args, model, tokenizer, data_file, pool, mode='dev'):
    """ Evaluate the model """
    ...
    if (mode == 'test'):
        with open(os.path.join(args.output_dir,"predictions_for_test.txt"),'w') as f:
            for example, pred in zip(eval_dataset.examples, y_preds):
                if pred:
                    f.write(' '.join(example.input_tokens)+'\t'+'1'+'\n')
                else:
                    f.write(' '.join(example.input_tokens)+'\t'+'0'+'\n')
...
def main():
    ...
    if args.do_eval:
        ...
        result = evaluate(args, model, tokenizer, args.eval_data_file, pool = pool, mode='dev')
    ...
    if args.do_test:
        ...
        model_to_load = model.module if hasattr(model, 'module') else model  
        model_to_load.load_state_dict(torch.load(output_dir))       
        result = evaluate(args, model, tokenizer, args.test_data_file, pool = pool, mode='test')
