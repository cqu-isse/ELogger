...
import os 
os.environ["CUDA_VISIBLE_DEVICES"]="2,3" # 使用编号为0,1号的GPU
...
def convert_examples_to_features(item):
    ...
    source_tokens_1,source_ids_1,position_idx_1,dfg_to_code_1,dfg_to_dfg_1=source_tokens,source_ids,position_idx,dfg_to_code,dfg_to_dfg
  
    return InputFeatures(source_tokens_1,source_ids_1,position_idx_1,dfg_to_code_1,dfg_to_dfg_1,label)
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    with open(os.path.join(args.output_dir,"predictions.txt"),'w') as f:
        for example,pred in zip(eval_dataset.examples,y_preds):
            if pred:
                f.write(' '.join(example.input_tokens_1)+'\t'+'1'+'\n')
            else:
                f.write(' '.join(example.input_tokens_1)+'\t'+'0'+'\n')
